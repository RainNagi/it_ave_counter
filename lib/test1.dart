...
class StatisticsPage1 extends StatefulWidget {
  const StatisticsPage1({super.key});

  @override
  _StatisticsPageState createState() => _StatisticsPageState();
}

class _StatisticsPageState extends State<StatisticsPage1> {
  @override
  void initState() {
    super.initState();
    fetchYears();
  ...
  Future<void> fetchStatistics() async {
    final url = Uri.parse('http://$ip/kpi_itave/statistics1.php?year=$selectedYear&month=$selectedMonthValue');
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        List<dynamic> data = jsonDecode(response.body);
        setState(() {
          statistics = {for (var item in data) item["button_id"]: item["occurrences"]};
          print(data);
        });
      } else {
